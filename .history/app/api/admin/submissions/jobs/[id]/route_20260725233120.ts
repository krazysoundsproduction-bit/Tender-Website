...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const jobPayload = {
    ...
    // Fast Job & Pay fields
    is_fast_job: submission.is_fast_job ?? false,
    payment_amount: submission.payment_amount,
    completion_timeframe: submission.completion_timeframe,
    poster_name: submission.poster_name,
    poster_phone: submission.poster_phone,
    poster_email: submission.poster_email,
    preferred_contact: submission.preferred_contact,
  };

  // Use admin client to bypass RLS policies for creating approved jobs
  const adminClient = createAdminClient();
  const { data: createdJob, error: createError } = await adminClient
    .from("job_vacancies")
    .insert(jobPayload)
    .select("id")
    .single();
